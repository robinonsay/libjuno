...
JUNO_STATUS_T EngineTlmMsgPipeInit(ENGINE_TLM_MSG_PIPE_T *ptCmdPipe, JUNO_FAILURE_HANDLER_T pfcnFailureHdlr, JUNO_USER_DATA_T *pvUserData)
{
    ...
    JUNO_STATUS_T tStatus = JunoSb_PipeInit(&ptCmdPipe->tRoot, &gtCmdPipeApi, ENGINE_TLM_MSG_MID, ENGINE_TLM_MSG_PIPE_DEPTH, pfcnFailureHdlr, pvUserData);
    return tStatus;
}
...
static JUNO_STATUS_T SetAt(JUNO_DS_ARRAY_ROOT_T *ptArray, JUNO_POINTER_T tItem, size_t iIndex)
{
    ...
    JUNO_ASSERT_SUCCESS(tStatus, return tStatus);
    JUNO_ASSERT_POINTER(tStatus, tItem, ENGINE_TLM_MSG_T, gtEngineTlmMsgPointerApi);
    tStatus = JunoDs_ArrayCheckIndex(ptArray, iIndex);
    JUNO_ASSERT_SUCCESS(tStatus, return tStatus);
    ENGINE_TLM_MSG_PIPE_T *ptCmdPipe = (ENGINE_TLM_MSG_PIPE_T *)ptArray;
    JUNO_POINTER_T tIndexPointer = JunoMemory_PointerInit(&gtEngineTlmMsgPointerApi, ENGINE_TLM_MSG_T, &ptCmdPipe->ptArrCmdPipe[iIndex]);
