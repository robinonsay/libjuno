...
JUNO_STATUS_T Juno_TablePosixLoad(JUNO_TABLE_MANAGER_T *ptTableManager)
{
    ...
    size_t zBytesRead = fread(ptTableManager->ptBuff, ptTableManager->zBuffSize, 1, ptFile);
    int iErrStatus = ferror(ptFile);
    fclose(ptFile);
    if(zBytesRead != ptTableManager->zBuffSize)
    {
        if(zBytesRead == 0 && iErrStatus)
        {
...
JUNO_STATUS_T Juno_TablePosixSave(JUNO_TABLE_MANAGER_T *ptTableManager)
{
    ...
    size_t zBytesRead = fwrite(ptTableManager->ptBuff, ptTableManager->zBuffSize, 1, ptFile);
    int iErrStatus = ferror(ptFile);
    fclose(ptFile);
    if(zBytesRead != ptTableManager->zBuffSize)
    {
