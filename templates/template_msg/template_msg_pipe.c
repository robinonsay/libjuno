...
JUNO_STATUS_T TemplateMsgPipeInit(TEMPLATE_MSG_PIPE_T *ptCmdPipe, JUNO_FAILURE_HANDLER_T pfcnFailureHdlr, JUNO_USER_DATA_T *pvUserData)
{
    ...
    JUNO_STATUS_T tStatus = JunoDs_QueueInit(&ptCmdPipe->tRoot.tRoot, &gtCmdPipeApi, TEMPLATE_MSG_PIPE_DEPTH, pfcnFailureHdlr, pvUserData);
    return tStatus;
}
...
static JUNO_STATUS_T SetAt(JUNO_DS_ARRAY_ROOT_T *ptArray, JUNO_POINTER_T tItem, size_t iIndex)
{
    ...
    JUNO_ASSERT_SUCCESS(tStatus, return tStatus);
    JUNO_ASSERT_POINTER(tStatus, tItem, TEMPLATE_MSG_T, gtTemplateMsgPointerApi);
    tStatus = JunoDs_ArrayCheckIndex(ptArray, iIndex);
    JUNO_ASSERT_SUCCESS(tStatus, return tStatus);
    TEMPLATE_MSG_PIPE_T *ptCmdPipe = (TEMPLATE_MSG_PIPE_T *)ptArray;
    JUNO_POINTER_T tIndexPointer = JunoMemory_PointerInit(&gtTemplateMsgPointerApi, TEMPLATE_MSG_T, &ptCmdPipe->ptArrCmdPipe[iIndex]);
