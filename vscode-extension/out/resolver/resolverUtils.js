...
exports.lookupVariableType = lookupVariableType;
exports.walkToRootType = walkToRootType;
exports.parseIntermediates = parseIntermediates;
/**
 ...
 */
function findEnclosingFunction(index, file, line) {
    let bestLine = -1;
    let bestName;
 ...
 */
function lookupVariableType(index, file, funcName, varName) {
 ...
 */
function walkToRootType(index, typeName) {
 ...
 */
function parseIntermediates(chainStr) {
    if (!chainStr.trim()) {
        return [];
    }
    const re = /(?:->|\.)\s*(\w+)/g;
