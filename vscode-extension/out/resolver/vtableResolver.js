...
class VtableResolver {
    ...
    resolve(file, line, column, lineText, functionName) {
        ...
        const macroRe = /JUNO_MODULE_GET_API\s*\(\s*\w+\s*,\s*(\w+)\s*\)\s*->\s*(\w+)\s*\(/g;
        let m;
        ...
        const arrayRe = /(\w+)\s*\[.*?\]((?:\s*(?:->|\.)\s*\w+)*)\s*->\s*(\w+)\s*\(/g;
        ...
        const generalRe = /(\w+)((?:\s*(?:->|\.)\s*\w+)*)\s*->\s*(\w+)\s*\(/g;
     ...
     * fallback.
     */
    resolveChain(file, enclosingFunc, primary, chainStr, fieldName) {
        const intermediates = (0, resolverUtils_1.parseIntermediates)(chainStr);
        const apiTypeCandidates = [];
        // Candidate 1: resolve from the primary identifier's declared type.
        if (enclosingFunc) {
            const typeInfo = (0, resolverUtils_1.lookupVariableType)(this.index, file, enclosingFunc, primary);
            if (typeInfo) {
                ...
                if (typeName.endsWith('_API_T')) {
                    ...
                    const rootType = (0, resolverUtils_1.walkToRootType)(this.index, typeName);
                    const apiType = this.index.moduleRoots.get(rootType) ??
     ...
     * needed.
     */
    resolveByRootField(rootType, fieldName) {
        const walkedRoot = (0, resolverUtils_1.walkToRootType)(this.index, rootType);
        const apiType = this.index.moduleRoots.get(walkedRoot) ??
     ...
     */
    lookupVtable(apiType, fieldName) {
...
exports.VtableResolver = VtableResolver;
//# sourceMappingURL=vtableResolver.js.map